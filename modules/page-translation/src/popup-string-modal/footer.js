...
const StringPopUpFooter = (props) => {
    ...
    return (
        <div className="modal-footer" key={props.modalRender}>
            {!props.translatePendingStatus && <StringPopUpNotice className="lmat_page_translation_string_count"><p>{__('Wahooo! You have saved your valuable time via auto translating', 'translate-words')} <strong><FormatNumberCount number={props.characterCount} /></strong> {__('characters using', 'translate-words')} <strong>{props.serviceLabel}</strong>.{__('Please share your feedback —', 'translate-words')}<a href="https://wordpress.org/support/plugin/translate-words/reviews/#new-post" target="_blank" rel="noopener" style={{color: 'yellow'}}>{__('leave a quick review', 'translate-words')}</a>!</p></StringPopUpNotice>}
