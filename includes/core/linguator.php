...
class Linguator {
	...
	public function __construct() {
		...
		// Share one CPFM host with LocoAI / AutoPoly (category: cool_translations).
		...
		if ( is_admin() ) {
			if ( ! class_exists( 'CPFM_Feedback_Notice', false ) ) {
				...
				if ( ! class_exists( 'CPFM_Feedback_Notice', false ) ) {
					class_alias( \Linguator\Admin\cpfm_feedback\CPFM_Feedback_Notice::class, 'CPFM_Feedback_Notice' );
				}
			}
			...
			// Keep lmat_feedback_data in sync when the shared CPFM option is set (including first "No").
			$sync_lmat_feedback_data = static function ( $value ) {
				$option = get_option( 'linguator' );
				if ( ! is_array( $option ) ) {
					return;
				}
			...
			add_action(
				'add_option_cpfm_opt_in_choice_cool_translations',
				static function ( $option, $value ) use ( $sync_lmat_feedback_data ) {
					$sync_lmat_feedback_data( $value );
				},
				10,
				2
			);
			...
			add_action(
				'update_option_cpfm_opt_in_choice_cool_translations',
				static function ( $old_value, $value ) use ( $sync_lmat_feedback_data ) {
					$sync_lmat_feedback_data( $value );
				},
				10,
				2
